  ...
  useEffect(() => {
    ...
    const data = labels.map((label, i) => {
      ...
      const count = gender.filter((phone) => {
        const phoneNumber = parseInt(phone.substring(0, 3));
        return phoneNumber >= lowerBound && phoneNumber <= upperBound;
      }).length;
      return count;
    });
  ...
  return (
    <CRow>
      <CCol>
        <CCol xs={12}>
          <CCard>
            <CCardHeader className="bg-dark p-3" xs={12}>
              <div className="row">
        ...
        <CRow>
          ...
          <CCol xs={12} lg={4}>
            <CCard className="mb-4">
              <CCardHeader> Phone Number Provider Doughnut Chart</CCardHeader>
              <CCardBody>
